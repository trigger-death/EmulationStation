 ...
 */
TEST(GameDataGame, Empty) {
	GameDataGame	gdi(nullptr, "", "", "", 1, 5);
 ...
 */
TEST(GameDataGame, Data) {
	GameDataGame	gdi(nullptr, "1942", "arcade", "1-9/1942.zip", 4, 123);
 ...
 */
TEST(GameDataGame, Tags) {
	MockGameDatabase db;
	db.create();
	db.addGame("rtype", "arcade", "~/Mame/rtype.zip", "", "3", "0");
	db.addTag("rtype", "arcade", "scroller");

	GameDataGame	gdi(db.mDB, "rtype", "arcade", "r/rtype.zip", 4, 123);
